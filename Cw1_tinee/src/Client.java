...
public class Client {
  ...
  boolean printSplash = true; // this printSplash is related to the UI and t is coupling the UI and the bussness logic
                             // the MC should be used to decouple the bussiness logic and the UI                                    

  Client() {
  } 
//initialzer is not being used this is bad becouse th user host and port variable are being set in the set methord 
  ...
  public static void main(String[] args) throws IOException {
    ...
    client.set(user, host, port); // add to line 82
    client.run();
  } 
// check if argumets are null then if null we need to prompt the user to input data for the requiered data
  ...
  void run() throws IOException {
    ...
    try {
      ...
      if (this.user.isEmpty() || this.host.isEmpty()) {              // clients 
        System.err.println("User/host has not been set.");
        System.exit(1);                                        // this bad becuase it will shut down the whole JVM it could be improved by giving a error message instead of shutting down the JVM
      }
      helper = new CLFormatter(this.host, this.port);

      if (this.printSplash = true);                               // if statement would run regardless as there is a semicolon and it can be corrected by removing the semi colon
      {
...
// Main loop: print user options, read user input and process
  void loop(CLFormatter helper, BufferedReader reader) throws IOException,
    ...
    String state = "Main";  // Initial state 
                                                         // all strings need to be internationalized 
    ...
    // The loop
    for (boolean done = false; !done;) {        // whole loop function is carrying out too many resposibilites and can be broken down inot individual methods 
                                                // stacks of if statments which is hard to read unmanitianable  and solution to this would be to use command pattern  

      // Print user options
      if (state.equals("Main")) {                                   // the state is being put first so it might crash it could be resolved by putting the main first in theif statement 
